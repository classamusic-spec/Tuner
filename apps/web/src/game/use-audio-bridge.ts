  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      for (const enemy of state.enemies) {
        ...
        if (dx * dx + dz * dz <= COMBAT_PRESSURE_RADIUS * COMBAT_PRESSURE_RADIUS) nearby++;
      }
      ...
      director.setSignals({
        region: state.stage.stageId as MusicRegion,
        phase: phase as MusicPhase,
        ...
        coherenceFraction:
          player.maxCoherence > 0 ? player.coherence / player.maxCoherence : 1,
