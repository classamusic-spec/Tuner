...
import { createWebAudioEngine, type AudioEngine } from '@tuner/audio';
import { createGameCore, type AccessibilityConfig, type GameCore, type GameEvents } from '@tuner/game-core';
...
export interface GameLoopHandle {
  ...
  readonly alphaRef: { current: number };
  /** Camera yaw/pitch owned by the renderer and fed back into aiming. */
  readonly cameraRef: { current: { yaw: number; pitch: number } };
...
export function useGameLoop(options: GameLoopOptions): GameLoopHandle {
  const [ready, setReady] = useState(false);
  const [error, setError] = useState<Error | null>(null);

  const eventsRef = useRef<EventBus<GameEvents>>(createEventBus<GameEvents>());
  ...
  useEffect(() => {
    if (audioUnlocked) void audioRef.current?.unlock();
  }, [audioUnlocked]);
  ...
  const start = useCallback(
    (stageId: StageId): void => {
      const core = coreRef.current;
      const input = inputRef.current;
      if (!core || !input || runningRef.current) return;
  ...
  const setPaused = useCallback((paused: boolean): void => {
    ...
    if (paused) audioRef.current?.suspend();
