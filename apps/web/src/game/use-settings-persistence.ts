...
import { useUIStore, type UIState } from '@tuner/ui';
...
interface PersistedSettings {
  readonly difficulty: UIState['difficulty'];
  readonly graphicsTier: UIState['graphicsTier'];
  readonly audioLevels: UIState['audioLevels'];
  readonly inputSettings: UIState['inputSettings'];
  readonly accessibility: UIState['accessibility'];
}

export function useSettingsPersistence(): { readonly loaded: boolean } {
    ...
    void (async () => {
      try {
        ...
        if (!cancelled && raw !== null) {
          ...
          if (parsed.difficulty) store.setDifficulty(parsed.difficulty);
          if (parsed.graphicsTier) store.setGraphicsTier(parsed.graphicsTier);

          if (parsed.audioLevels) {
            for (const [bus, value] of Object.entries(parsed.audioLevels)) {
              if (typeof value === 'number') {
                store.setAudioLevel(bus as keyof UIState['audioLevels'], value);
              }
            }
          }
          if (parsed.inputSettings) {
            for (const [key, value] of Object.entries(parsed.inputSettings)) {
              store.setInputSetting(
                key as never,
                value as never,
              );
            }
          ...
          if (parsed.accessibility) {
            for (const [key, value] of Object.entries(parsed.accessibility)) {
              store.setAccessibility(key as never, value as never);
            }
          }
        }
      } catch {
